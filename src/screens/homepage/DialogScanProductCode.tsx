...
export function DialogScanProductCode() {
  ...
  useEffect(() => {
    const processedStr = extractStringForProductCode(newReactHookFormVal); // prettier-ignore
    const isVerifiedProductCode = selectedFormulationReportLines.productCode === processedStr.productCode; // prettier-ignore

    if (processedStr.productDate !== '') {
      setProductBatchNumber(processedStr.productDate);
    }
